...
class ImportCategory implements ToModel, WithHeadingRow
{
   ...
   { 
	  ...
	  if(isset($row['item_name']) && $row['item_name']!=''){
		    if($row['item_type']=='MODEL'){ $product_type="Single"; }else{ $product_type="Option"; }
			//check product exist or not
		   $product=Product::where('name',trim($row['item_name']))->where('product_type',$product_type)->first();
		   if(isset($product['id'])){
