...
<section class="py-5">
	...
	<?php if(isset($error)){ echo $error; }else{ ?>
		<div class="ctm-prd-one relative overflow-hidden py-[30px] lg:py-[50px] px-[20px] lg:px-[70px] bg-no-repeat bg-center bg-cover min-h-[200px] md:min-h-[350px] flex justify-center rounded-[20px] after:content-['*'] after:absolute after:top-0 after:bottom-0 after:left-0 after:right-0 after:bg-[#000]/[.5] after:w-full after:h-full" style="background-image: url('<?php echo url('storage/' . $category['image']); ?>')">
                <img src="{{ asset('img/small-logo.png') }}" class="h-[44px] max-w-[44px] contain absolute right-[10px] top-[10px] z-[1]" alt="">
                <h3 class="text-[#fff] text-[25px] md:text-[52px] uppercase font-[800] leading-[32px] md:leading-[62px] z-[1]">{{ $category['name'] ?? '' }}</h3>
