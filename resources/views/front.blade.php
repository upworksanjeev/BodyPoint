	...
	<?php if(isset($error)){ echo $error; }else{ ?>
          ...
          <div class="prodct-Category mt-[15px] md:mt-[30px]">
            ...
            <div>
			  ...
			  @foreach ($products as $prod)
                <div class="relative bg-[#fff] rounded-[15px] p-5 border border-[#ECECEC] h-[auto]" @click="redirectprod('<?php echo str_replace(' ','-',strtolower(strip_tags($prod['product']['name']))); ?>')">
                  ...
                  <h6 class="text-[18px] text-[#253D4E] mb-2 mt-3 font-[600]" @click="redirectprod('<?php echo str_replace(' ','-',strtolower(strip_tags($prod['product']['name']))); ?>')">{{ $prod['product']['name'] ?? '' }}</h6>
