                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-9">
                    <div class="">
                        <div class="card bg-white border border-gray-200 rounded-2xl shadow mb-4">
                            <div
                                class="card-header px-6 py-4  flex gap-2  sm:gap-0 flex-col sm:flex-row items-start  sm:items-center justify-start sm:justify-between border-b ">
                                ...
                                <ul class="max-w-md space-y-5 text-gray-500 list-disc list-inside mb-8">
                                    <li class="flex items-start gap-5">
                                        ...
                                        <span class="text-sm text-[#000] font-normal leading-[17px]">
                                            {{ session()->get('customer_details') ? session()->get('customer_details')['CustomerName'] : '' }}</span>
                                    </li>
                                    <li class="flex items-start gap-5">
                                        <span class="text-sm text-[#000] font-normal leading-[17px]">Address:</span>
                                        <span
                                            class="text-sm text-[#000] font-normal leading-[17px] change-shipping-address">
                            ...
                            <div class="flex items-center mb-1 sm:mb-4">
                                <input id="default-checkbox" type="checkbox" value=""
                                    class="w-4 h-4 text-[#00707B]-600 bg-gray-100 border-gray-300 rounded focus:ring-[#00707B]-500 focus:ring-2">
                                <label for="default-checkbox" class="ms-2 text-sm font-medium text-[#00707B]">Make This
                    ...
                    <div>
                        <a onclick="addToCart()"
                            class="py-2.5 px-5 text-sm font-medium text-white focus:outline-none bg-[#FF9119] rounded-full border border-[#FF9119] focus:z-10 focus:ring-4 focus:ring-[#FF9119]/40 flex gap-3 items-center hover:bg-[#FF9119]/80 justify-center w-[160px] float-right"
                            href="{{ route('payment') }}">
                            Next
                        </a>
                    </div>
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-4 md:p-5 space-y-4">
                    <ul class="grid w-full gap-6 md:grid-cols-2">
                        @foreach (session('customer_details')['ShipToAddresses'] as $key => $address)
                            <li>
                                <input type="radio" id="shipping-radio-{{ $key }}" name="shipping"
                                    class="hidden peer shipping-radio-class" required data-key="{{ $key }}" />
                                <label for="shipping-radio-{{ $key }}"
                                    class="inline-flex items-center justify-between w-full p-3 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-gray-600 peer-checked:text-gray-600 hover:text-gray-600 hover:bg-gray-100">
                                    <div class="block">
                                        <div class="w-full text-normal font-semibold mb-2">
                                            {{ session('customer_details')['CustomerName'] }}</div>
                                        <div
                                            class="w-full text-sm font-normal leading-[17px] space-y-1 shipping-address-{{ $key }}">
                ...
                <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b">
                    <button data-modal-hide="default-modal" type="button"
                        class="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-[#000000] hover:bg-[#00838f] hover:border-[#027480] hover:text-[#fff] focus:z-10 focus:ring-4 focus:ring-gray-100 flex gap-3 items-center justify-center w-[100px]">Close</button>
                    <button data-modal-hide="default-modal" type="button" id="updateShippingAddress"
