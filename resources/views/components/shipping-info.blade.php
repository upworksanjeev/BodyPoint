         ...
         <div class="card-body p-6">
             <ul class="max-w-md space-y-5 text-gray-500 list-disc list-inside">
                 ...
                 <li class="flex items-start gap-5">
                     ...
                     <span class="text-sm text-[#000] font-normal leading-[17px] change-shipping-address">{{ session('customer_address')['AddressLine1'] ?? session('customer_details')['ShipToAddresses'][0]['AddressLine1'] }} <br>{{ session('customer_address')['AddressLine2'] ?? session('customer_details')['ShipToAddresses'][0]['AddressLine2'] }} {{ session('customer_address')['AddressLine3'] ?? session('customer_details')['ShipToAddresses'][0]['AddressLine3'] }} <br> {{ session('customer_address')['State'] ?? session('customer_details')['ShipToAddresses'][0]['State'] }} {{ session('customer_address')['PostalCode'] ?? session('customer_details')['ShipToAddresses'][0]['PostalCode'] }} {{ session('customer_address')['Country'] ?? session('customer_details')['ShipToAddresses'][0]['Country'] }}</span>
                 </li>
                 <li class="flex items-start gap-5">
                     <span class="text-sm text-[#000] font-normal leading-[17px] w-[55px]">Country</span>
                     <span class="text-sm text-[#000] font-normal leading-[17px]">{{ $userDetail->country ?? ''}}</span>
                 </li>
                 <li class="flex items-start gap-5">
