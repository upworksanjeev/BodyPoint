        ...
        <div
            ...
            <div style="padding: 24px;">
                ...
                <div style="display: flex; gap: 20px; align-items: center; margin-top: 20px;">
                    <span style="line-height: 17px; color: #000; font-size: 14px; font-weight: 400;">Purchase
                        Order #:</span>
                    <span
                        style="background-color: #31ba32; font-size: 14px; font-weight: 500; line-height: 20px; padding: 2px 20px; border-radius: 100px; color: #fff;">{{ $quote->purchase_order_no }}</span>
                </div>
                <div style="display: flex; gap: 20px; align-items: center; margin-top: 20px;">
                    <span style="line-height: 17px; color: #000; font-size: 14px; font-weight: 400;">Order
                        Date:</span>
                    <span
                        style="line-height: 17px; color: #000; font-size: 14px; font-weight: 400;">{{ date('F j, Y', strtotime($quote->created_at)) }}</span>
                </div>
                <div style="display: flex; gap: 20px; align-items: center; margin-top: 20px;">
                ...
                <div
                    ...
                    <table style="border-collapse: collapse;width: 100%;" class="productDetail">
                        ...
                        <tbody>
                            ...
                            // Find processed items for this quote if available
                            $currentQuoteProcessedItems = null;
                            if (isset($quotesWithComments)) {
                                foreach ($quotesWithComments as $quoteWithComments) {
                                    if ($quoteWithComments['quote']->id == $quote->id) {
                                        $currentQuoteProcessedItems = $quoteWithComments['processedItems'];
                                        break;
                                    }
                                }
                            }
                            @endphp
                            @if (!empty($quote))
                                @if ($currentQuoteProcessedItems && !empty($currentQuoteProcessedItems))
                                    @foreach ($currentQuoteProcessedItems as $processedItem)
                                        @php
